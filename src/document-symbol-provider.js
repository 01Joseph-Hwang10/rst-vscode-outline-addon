/**
 * @fileoverview
 * This file is basically a copy-paste from [trond-snekvik.simple-rst](https://marketplace.visualstudio.com/items?itemName=trond-snekvik.simple-rst) extension.
 */
  ...
  provideDocumentSymbols(document, token) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        !line.match(
          new RegExp(
            `^(${underlines.map((char) => char + "{" + prev.length + ",}").join("|")})\\s*$`,
          ),
        )
      ) {
        prev = line;
    ...
    const createDef = (section) => {
      const sym = new vscode.DocumentSymbol(
        ...
        new vscode.Range(section.start, 0, section.end, 99999),
        new vscode.Range(
          section.start,
          0,
          section.start + 1,
